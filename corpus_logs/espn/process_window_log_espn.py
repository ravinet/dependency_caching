...
for (a,b) in final_dependencies:
  if ( a != b ):
    parseda = urlparse.urlparse(a)
    parsedb = urlparse.urlparse(b)
    parent_name = parseda.path.split("/")[-1]
    if ( parseda.query != '' ):
        parent_name = parent_name + "?" + parseda.query
    child_name = parsedb.path.split("/")[-1]
    if ( parsedb.query != '' ):
        child_name = child_name + "?" + parsedb.query
    if ( a == "/" ):
        parent_name = a
    if ( b == "/" ):
        child_name = b
    if ( "?" in parent_name ):
        temp = parent_name
        if ( temp[0:2] == "c?"):
            parent_name = "c?" + temp.split("?")[1].split("/")[-1].split("&")[0]
        elif (temp[0:12] == "201303290924"  or temp[0:2] == "3?"):
            parent_name = temp.split("/")[1]
        elif ( temp[0:2] == "i?"):
            parent_name = temp.split("/")[-1].split("&")[0]
        else:
            parent_name = temp.split("?")[0]
    if ( "?" in child_name ):
        temp = child_name
        if ( temp[0:2] == "c?"):
            child_name = "c?" + temp.split("?")[1].split("/")[-1].split("&")[0]
        elif (temp[0:12] == "201303290924"  or temp[0:2] == "3?"):
            child_name = temp.split("/")[1]
        elif ( temp[0:2] == "i?"):
            child_name = temp.split("/")[-1].split("&")[0]
        else:
